...
parser = argparse.ArgumentParser(
    description='Manages the GOV.UK Notify"\
                                "Orchestration API'
)

parser.add_argument(
    "--host", type=str, help=f"(Optional) Sets which host to run the API on, " f"defaults to {Config.HOST}"
)

parser.add_argument(
    "--port", type=int, help=f"(Optional) Sets which port to run the API on, " f"defaults to {Config.PORT}."
)

parser.add_argument(
    "--log-level", type=int, help="(Optional) Sets which log level to display, " "defaults to 'debug'."
)
